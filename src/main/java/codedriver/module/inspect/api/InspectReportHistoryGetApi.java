...
@Service
public class InspectReportHistoryGetApi extends PrivateApiComponentBase {
    @Resource
    ...
    @Input({@Param(name = "id", type = ApiParamType.STRING, desc = "id", isRequired = true)})

    @Description(desc = "根据历史Id 获取对应的巡检报告")
    @Override
    public Object myDoService(JSONObject paramObj) throws Exception {
        MongoCollection<Document> collection = mongoTemplate.getDb().getCollection("INSPECT_REPORTS_HIS");
        Document doc = new Document();
        doc.put("_id", new ObjectId(paramObj.getString("id")));
    ...
    public String getToken() {
        return "inspect/report/history/get";
    }
}
