...
//        }
    ...
    public String getResourceIdListSql(InspectResourceSearchVo searchVo, List<ResourceInfo> unavailableResourceInfoList) {
    ...
    public String getResourceCountSql(InspectResourceSearchVo searchVo, List<ResourceInfo> unavailableResourceInfoList) {
        //查出资源中心数据初始化配置信息来创建ResourceSearchGenerateSqlUtil对象
        IResourceCenterResourceCrossoverService resourceCenterResourceCrossoverService = CrossoverServiceFactory.getApi(IResourceCenterResourceCrossoverService.class);
        List<ResourceEntityVo> resourceEntityList = resourceCenterResourceCrossoverService.getResourceEntityList();
        ResourceSearchGenerateSqlUtil resourceSearchGenerateSqlUtil = new ResourceSearchGenerateSqlUtil(resourceEntityList);
...
//        theadList.add(new ResourceInfo("resource_softwareservice_env", "env_name"));
     ...
     * @return
     */
    private PlainSelect getPlainSelectBySearchCondition(InspectResourceSearchVo searchVo, ResourceSearchGenerateSqlUtil resourceSearchGenerateSqlUtil, List<ResourceInfo> unavailableResourceInfoList) {
        PlainSelect plainSelect = resourceSearchGenerateSqlUtil.initPlainSelectByMainResourceId("resource_ipobject");
        if (plainSelect == null) {
            return null;
        }
...
//            AndExpression andExpression = new AndExpression(equalsTo2, equalsTo3);
            EqualsTo equalsTo4 = new EqualsTo()
                    .withLeftExpression(new Column(table2, "job_id"))
                    .withRightExpression(new LongValue(searchVo.getJobId()));
            AndExpression andExpression = new AndExpression(equalsTo2, equalsTo4);
            ...
            Join join2 = new Join().withRightItem(table2).addOnExpression(andExpression);
            plainSelect.addJoins(join2);
        }
